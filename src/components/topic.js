...
import {fetchRepliesIfNeeded, fetchTopicIfNeeded, invalidateReplies} from '../actions'
import SiderBar from './siderbar'
import '../assets/styles/topics.css'
import '../assets/styles/markdown.css'
import moment from 'moment'
import ReplyItem from './replyItem.js';
import {Alert, Col, Grid, Panel, ProgressBar, Row} from 'react-bootstrap'
...
class Topic extends Component {
    ...
    render() {
        ...
        const repliesUserContainer = (
            topic ? (topic.replies_count === 0 ? '' : <span> · 最后由 <Link
                to={'/users/' + topic.last_reply_user_id}>{topic.last_reply_user_login}</Link> 回复于 {moment(topic.replied_at).fromNow()} </span>) : ''
        );
            ...
            <div className="detail-header">
                <div className="header-infos">
                    <div className="header-title"><Link
                        to={"/topic?node_id" + topic.node_id}>{topic.node_name}</Link>{topic.title}</div>
                    <div className="header-info">
                        <Link
                            to={topic.user ? '/users/' + topic.user.id : ''}>{topic.user ? topic.user.login : ''}</Link>
                ...
                <div className="header-avatar">
                    <Link to={topic.user ? '/users/' + topic.user.id : ''}><img alt={topic.user ? topic.user.login : ''}
        ...
        return (
            <Grid>
                <Row>
                    <Col md={9}>
                        {topicIsFetching
                            ? <div style={{width: '80%', margin: '0 auto'}}><ProgressBar active now={45} label="努力加载中"/>
                            </div>
                            : container
                        }

                        <div className="topic-replies">
                            <div className="replies-header">共收到 <strong>{topic ? topic.replies_count : 0}</strong> 条回复
                            </div>
                            <div className="replies-body">
                                {repliesIsFetching
                                    ? <div style={{width: '80%', margin: '0 auto'}}><ProgressBar active now={45}
                                                                                                 label="努力加载中"/></div>
                                    : repliesContainer
                                }
                            </div>
