            ...
            this.state.current.indexOf('topics') > -1 && this.state.node_id === 0
                ? <div id="node-header">
                    <Grid>
                        <ul className="nav nav-pills node-filter">
                            <li><Button bsStyle="default" className="node-button" onClick={this.handleClick}>所有节点<Font
                                name="caret-right"/></Button></li>
                            <li className={this.state.current === '/topics' ? 'active' : ''}><Link to='/topics'>默认</Link>
                            </li>
                            <li className={this.state.current === '/topics/popular' ? 'active' : ''}><Link
                                to='/topics/popular'>
                                <div type="smile-o"/>
                                优质帖子</Link></li>
                            <li className={this.state.current === '/topics/no_reply' ? 'active' : ''}><Link
                                to='/topics/no_reply'>无人问津</Link></li>
                            <li className={this.state.current === '/topics/last' ? 'active' : ''}><Link to='/topics/last'>最新发布</Link>
                            </li>
                        </ul>
                    </Grid>
                </div>
                : (this.state.current === '/jobs' || this.state.current === '/homeland' || this.state.node_id > 0
                    ? <Panel className="node-panel">
                        <Grid>
                            <div className="node-title">{this.state.node ? this.state.node.name : ''}<span style={{
                                fontSize: '14px',
                                color: '#999',
                                marginLeft: '10px'
                            }}>共有{this.state.node ? this.state.node.topics_count : 0}个讨论主题</span></div>
                            <div className="node-summry">
                                <ReactMarkdown
                                    source={this.state.node && this.state.node.summary ? this.state.node.summary : ''}/>
                            </div>
                        </Grid>
                    </Panel>
                ...
                <Grid>
                    <Modal show={this.state.visible} bsSize="large" onHide={this.handleClick}>
                        <Modal.Header closeButton><Modal.Title
                            id="contained-modal-title-lg">选择话题节点</Modal.Title></Modal.Header>
                        <Modal.Body>
                            {nodes.map((node, i) =>
